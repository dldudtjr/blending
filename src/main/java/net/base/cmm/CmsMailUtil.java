...
public class CmsMailUtil {
    ...
    public void sendMailCertify(MailVO vo) { // 제목과 내용만을 보내는 기본 발송입니다.
        ...
        try {
            ...
            if("ko".equals(vo.getLang())){
                message.setSubject("[블렌딩트렌드] '" +vo.getSubject()+ "'이메일 인증 ", charSet);
            }else {
                message.setSubject("[BLENDING TREND] '" +vo.getSubject()+ "' Email certification", charSet);
            }
    ...
    public String makeMailFrm(MailVO vo) {
        ...
        if("en".equals(vo.getLang())){
            ...
            msgA="이(가)";
            msgB="의 복구 이메일로 표시되어 이 메시지가 발송되었습니다.";
            msgC="본인이 아닌 경우 아래 고객센터로 즉시 연락바랍니다.";
            msgD="블렌딩트렌드 비밀번호 찾기 안내";
            msgE="안녕하세요.";
            msgF="아래의 링크를 클릭하여 비밀번호를 변경해주세요.";
            msgG="비밀번호 변경하기 ";
    ...
    public static String getHostName(MailVO vo) {
        vo.setRtnUrl("http://" + EgovProperties.getProperty("hostName") + "/web/lgn/" + vo.getRtnVal() + "/usrInfoUdt.lp");
        return vo.getRtnUrl();
    }
